...
library TrieProofs {
    using RLP2 for RLP2.RLPItem;
    using RLP2 for bytes;
    ...
    ) external pure returns (bytes memory value)
    {
        ...
        RLP2.RLPItem[] memory proof = proofRLP.toRLPItem().toList();

        uint8 nodeChildren;
        RLP2.RLPItem memory children;
        ...
        for (uint256 i = 0; i < proof.length; i++) {
            ...
            // *rlp-encoded* items.
            bytes memory rlpNode = proof[i].toBytes(); // TODO: optimize by not encoding and decoding?
            ...
            RLP2.RLPItem[] memory node = proof[i].toList();
                ...
                */
                ...
                if (i == proof.length - 1) {
                    ...
                    if (!children.isList()) {
                        ...
                        nextHash = keccak256(children.toBytes());
                    }
                }
            } else {
    ...
    function getNextHash(RLP2.RLPItem memory node) internal pure returns (bytes32 nextHash) {
